document.addEventListener('DOMContentLoaded', function() {
    ...
    if (codeSnippetContainer) {
        codeSnippet = codeSnippetContainer.querySelector('code');
    }
    
    const copyCodeBtn = document.getElementById('copy-code');
    
    // Only add event listeners if all elements exist
    if (generateCodeBtn && codeLanguage && codeSnippet) {
    ...
    if (copyCodeBtn && codeSnippet) {
        copyCodeBtn.addEventListener('click', function() {
            ...
            // Use global toast if available, otherwise create local one
            if (window.showToast) {
